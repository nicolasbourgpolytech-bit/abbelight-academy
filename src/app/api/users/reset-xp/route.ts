...
export async function POST(request: Request) {
    try {
        ...
        // Reset XP
        await sql`UPDATE users SET xp = 0 WHERE id = ${dbUserId}`;

        // Reset Module Progress
        await sql`DELETE FROM user_module_progress WHERE user_email ILIKE ${dbUserEmail}`;

        // Reset Chapter Progress
        await sql`DELETE FROM user_progress WHERE user_email ILIKE ${dbUserEmail}`;
