...
export function DyeList({ dyes, onEdit, onDelete }: DyeListProps) {
    const proteins = dyes.filter(d => d.type === 'Fluorescent Protein');
    const organicDyes = dyes.filter(d => d.type !== 'Fluorescent Protein');

    const DyeGroup = ({ title, items }: { title: string, items: Dye[] }) => {
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {/* Optional badge if we want to show type explicitly on card too */}
                                    </div>
    ...
    return (
        <div className="space-y-6">
            <DyeGroup title="Fluorescent Proteins" items={proteins} />
            <DyeGroup title="Organic Dyes" items={organicDyes} />
        </div>
    );
}
