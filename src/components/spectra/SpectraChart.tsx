        ...
        <div className="flex flex-col space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="w-full lg:w-72 flex flex-col gap-3 pr-1">
                    ...
                    <div className="glass-card p-0 rounded-none h-full flex flex-col overflow-hidden">
                        {/* Dye Type Filters Header */}
                        <div className="p-4 border-b border-white/10 shrink-0 space-y-3 bg-white/5">
                            <div className="text-xs text-gray-400 font-medium uppercase tracking-wider">Filter Types</div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {categories.map(category => {
                                ...
                                if (category === 'UV') badgeColor = 'bg-purple-500';
                                if (category === 'Blue') badgeColor = 'bg-blue-500';
                                if (category === 'Green') badgeColor = 'bg-green-500';
                                if (category === 'Red') badgeColor = 'bg-red-500';
                                if (category === 'Far-red') badgeColor = 'bg-red-800';

                                if (categoryDyes.length === 0) return null; // Optional: hide empty categories if preferred, but user didn't specify. Keeping consistent with previous behavior logic but cleaner.

                                return (
                                    <div key={category} className="border-b border-white/10 last:border-0 hover:bg-white/5 transition-colors">
                                        <button
                                            onClick={() => toggleCategory(category)}
                                            className={`w-full flex items-center justify-between p-4 transition-all ${isOpen ? 'bg-white/5' : ''}`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-2 h-2 rounded-none ${badgeColor}`} />
                                            ...
                                            <div className="bg-black/20 border-t border-white/5">
                                                {categoryDyes.map(dye => (
                                                    <button
                                                        ...
                                                        className={`
                                                            w-full flex items-center gap-3 px-4 py-2.5 text-left transition-all text-xs border-l-[3px]
                                                            ${dye.visible
                                                ...
                                                {categoryDyes.length === 0 && (
                                                    <div className="p-3 text-xs text-gray-500 text-center italic">No dyes available</div>
                                                )}
                                            </div>
                                        )}
                                    </div>
