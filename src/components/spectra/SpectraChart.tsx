    ...
    const handleExport = async () => {
        ...
        try {
            if (chartContainerRef.current) {
                // Find the SVG element generated by Recharts
                const svgElement = chartContainerRef.current.querySelector('recharts-surface, svg');
                ...
                if (svgElement) {
                    ...
                    // Get dimensions from the container or SVG
                    const width = chartContainerRef.current.clientWidth;
                    const height = chartContainerRef.current.clientHeight;

                    // Scale for better resolution (e.g. 2x for Retina/Print)
                    const scale = 2;
                    ...
                    if (ctx) {
                        ...
                        img.onload = () => {
                            ...
                            downloadLink.download = `abbelight_spectra_config_${new Date().toISOString().slice(0, 10)}.png`;
